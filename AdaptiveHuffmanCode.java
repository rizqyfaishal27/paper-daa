...
//		writeTreeToFile(node, treeFileName);
	...
	public void updateTree(TreeNodeAdaptiveHuffman node) {
 		while(node != null) {
 			if(!node.isLeaf()) {
 				if(node.left.getWeight() > node.right.getWeight()) {
 					TreeNodeAdaptiveHuffman tempLeft = node.left;
 					node.left = node.right;
 					node.right = tempLeft;
 					int tempNodeNumber = node.left.getNodeNumber();
 					node.left.setNodeNumber(node.right.getNodeNumber());
 					node.right.setNodeNumber(tempNodeNumber);
 				}
 				node.setWeight(node.left.getWeight() + node.right.getWeight());
 			}
			node = node.parent;
		}
	}
